...
def render_results_page():
    """Render the Results page for loading runs and exploring outputs."""
                    ...
                    with ui.tab_panel(load_files_tab).classes('w-full h-full p-2 overflow-hidden'):
                        with ui.row().classes('w-full h-full gap-6 overflow-y-auto'):
                            ...
                            with ui.column().classes('flex-1 min-w-0 gap-4 items-stretch overflow-y-auto'):
                                ui.label('Select pickle files to load').classes('text-xs font-semibold')
                                ...
                                def load_pickles(restore_selection=False):
                                    selected_files = list(file_picker.value or [])
                                    if not selected_files:
                                        # Reset dependent state when all files are unselected.
                                        state.selected_result_files = []
                                        scenario_selector.options = []
                                        scenario_selector.value = []
                                        scenario_selector.update()
                                        baseline_selector.options = []
                                    ...
                                    actual_files = [
                                        pickle_files_map[name] for name in selected_files
                                        if name in pickle_files_map
                                    ]
                                    if not actual_files:
                                        ui.notify('Selected files could not be resolved.', type='warning')
                                        return

                                    state.selected_result_files = selected_files
                                    try:
                                        engine.load_pickle_files(actual_files)
                                    except Exception as exc:
                                        ui.notify(f'Failed to load result files: {exc}', type='negative')
                                        return
                                    ...
                                    with models_container:
                                        ...
                                        ui.label('Model results loaded').classes('text-xs font-semibold gap-2')
                                        for models_tuple, scen_list in sorted(
                                            ...
                                            if models_tuple:
                                                with ui.row().classes('w-full h-full gap-2 items-center'):
                                                    ...
                                                    ui.label(', '.join(scen_list)).classes('text-xs')
                                            else:
                            ...
                            with ui.column().classes('flex-1 items-start overflow-y-auto'):
                                ...
                                with ui.column().classes('w-[calc(50vw-8rem)]'):
                                    scenario_selector = ui.select(
                                        options=[],
                                        label='Scenarios',
                                        multiple=True,
                                        with_input=True
                                    ).classes('w-full overflow-auto').props('use-chips').bind_value(state, 'selected_scenarios')
                                    ...
                                    baseline_selector = ui.select(
                                        ...
                                        with_input=True,
                                        clearable=True
                                    ).classes('w-full overflow-auto').bind_value(state, 'selected_baseline')
    ...
    def update_dimensions():
        ...
        if not state.selected_variable:
            dims = ['Region', 'Technology', 'Category']
            for i, dim_name in enumerate(dims):
                with dimension_container:
                    with ui.column().classes('w-full h-full gap-1'):
                        ui.label(dim_name).classes('text-xs font-semibold')

                        dim_select = ui.select(
                            options=[],
                            label=f'Select {dim_name.lower()}',
                            multiple=True,
                            with_input=True,
                            clearable=True
        ...
        for i, dim_name in enumerate(dims):
            ...
            dim_label = engine.get_classification_label(dim_name)

            with dimension_container:
                with ui.column().classes('w-full h-full gap-1'):
                    ui.label(dim_label).classes('text-xs font-semibold')

                    # Multi-select for dimension values
                    dim_select = ui.select(
                        options=dim_values,
                        label=f'Select {dim_label.lower()}',
                        multiple=True,
                        with_input=True,
                        clearable=True
